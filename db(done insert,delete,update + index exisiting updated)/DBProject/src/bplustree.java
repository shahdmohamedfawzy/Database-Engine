...
public class bplustree implements Serializable {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3530547998840954999L;
	int m;
	InternalNode root;
	LeafNode firstLeaf;
	...
	//things that will be modified in class:
	//	 if index is string or double the tree needs to support it
		...
		Comparator<DictionaryPair> c = new Comparator<DictionaryPair>() {
			...
			public int compare(DictionaryPair o1, DictionaryPair o2) {
				...
				return a.compareTo(b);
					...
					String a = String.valueOf((String) o1.key);
					String b = String.valueOf((String)o2.key);
					
					return a.compareTo(b);
				}
				else {
					Double a = Double.valueOf((Double) o1.key);
					Double b = Double.valueOf((Double)o2.key);
					return a.compareTo(b);
				}
				}
	...
	//function of finding leafs khaled ayezha
	
	public ArrayList<Integer> getAllValues(Object key) {
		...
		ArrayList<DictionaryPair> result=Duplicate(dps,ln.numPairs , index);
		ArrayList<Integer> ValRes=new ArrayList<Integer>();
		
		for(DictionaryPair dp:result) {
			ValRes.add((int) dp.value);
		}
		return ValRes;
	}
	
	public ArrayList<Integer> getValues(Object key, String operation) {
	    List<LeafNode> leafNodes = new ArrayList<>();
	    // If B+ tree is empty, return null
	    if (isEmpty()) {
	        return null;
	    }

	    // Find leaf nodes that hold the dictionary key
	    if (this.root == null) {
	        LeafNode currNode = this.firstLeaf;
	        while (currNode != null) {
	            if (currNode.containsKey(key)) {
	    ...
	    for (LeafNode leafNode : leafNodes) {
	        ...
	        if (operation.equals("=")) {
	            if (index >= 0 && dps[index].key.equals(key)) {
	                values.add((int) dps[index].value);
	            }
	        } else if (operation.equals("<")) {
	            for (int j = 0; j < index; j++) {
	                values.add((int) dps[j].value);
	            }
	        } else if (operation.equals(">")) {
	            for (int j = index + 1; j < leafNode.numPairs; j++) {
	                values.add((int) dps[j].value);
	            }
	        } else if (operation.equals(">=")) {
	            for (int j = index; j < leafNode.numPairs; j++) {
	                values.add((int) dps[j].value);
	            }
	        } else if (operation.equals("<=")) {
	            for (int j = 0; j <= index; j++) {
		...
		public ArrayList<DictionaryPair> Duplicate(DictionaryPair[] dps, int numPairs, int index) {
	        ArrayList<DictionaryPair> result = new ArrayList<>();

	        if (index < 0 || index >= numPairs) {
	            return result;
	        }

	        Object keyToCheck = dps[index].key;
	        
	        // Search for duplicates after the provided index
	        for (int i = index + 1; i < numPairs; i++) {
	            if (dps[i].key.equals(keyToCheck)) {
	                result.add(dps[i]);
	            } else {
	                break; 
	            }
	        }
	        result.add(dps[index]); 
	        return result;
	    }
	 ...
	 */
		private LeafNode findLeafNode(Object key) {
	        ...
	        for (i = 0; i < this.root.degree - 1; i++) {
	            if (keys[i] instanceof Integer && key instanceof Integer) { // Modification: Check if both are Integer
	                if ((Integer) key < (Integer) keys[i]) {
	                    break;
	                }
	            } else if (keys[i] instanceof String && key instanceof String) { // Modification: Check if both are String
	                if (((String) key).compareTo((String) keys[i]) < 0) {
	                    break;
	                }
	            } else if (keys[i] instanceof Double && key instanceof Double) { // Modification: Check if both are Double
	                if ((Double) key < (Double) keys[i]) {
	                    break;
	                }
	            }
	        }
	...
	private void handleDeficiency(InternalNode in) {
		...
		// Borrow:
			...
			if (sibling.keys[0] instanceof Integer) {
				int borrowedKey = (Integer)sibling.keys[0];
				parent.keys[0] = borrowedKey;
			}
			else if (sibling.keys[0] instanceof String) {
				String borrowedKey = (String)sibling.keys[0];
				parent.keys[0] = borrowedKey;
			}
			else {
				Double  borrowedKey = (Double)sibling.keys[0];
				parent.keys[0] = borrowedKey;
			}
	 ...
	 *             pair
	 */
	public void delete(Object key, Double value) {
		if (isEmpty()) {
			...
			int dpIndex = -1;
		        for (int i = 0; i < ln.numPairs; i++) {
		            if (ln.dictionary[i].key.equals(key) && ln.dictionary[i].value == value) {  // Modified part
		                dpIndex = i;
		                break;
		            }
			if (dpIndex < 0) {
				...
				if (ln.isDeficient()) {
					...
					if (ln.leftSibling != null &&
						...
						if (borrowedDP.key instanceof Integer && parent.keys[pointerIndex - 1] instanceof Integer) {
							if (!(((Integer)borrowedDP.key) >= ((Integer)parent.keys[pointerIndex - 1]))) {
								parent.keys[pointerIndex - 1] = ln.dictionary[0].key;
							}
						}
						else if (borrowedDP.key instanceof Double && parent.keys[pointerIndex - 1] instanceof Double) {
						...
						// Update key in parent if necessary
						int pointerIndex = findIndexOfPointer(parent.childPointers, ln);
						if (borrowedDP.key instanceof Integer && parent.keys[pointerIndex] instanceof Integer ) {
							if (!(((Integer)borrowedDP.key) < ((Integer)parent.keys[pointerIndex]))) {
								parent.keys[pointerIndex] = sibling.dictionary[0].key;
							}
						}
						else if (borrowedDP.key instanceof Double && parent.keys[pointerIndex] instanceof Double ) {
	 ...
	 */
	public Double search(Object key) {
	 ...
	 */
	public ArrayList<Double> search(Object lowerBound, Object upperBound) {
		...
		while (currNode != null) {
			...
			for (DictionaryPair dp : dps) {
				...
				if(dp.key instanceof Integer && lowerBound instanceof Integer && upperBound instanceof Integer ) {
					if ((Integer)lowerBound <= ((Integer)dp.key) && ((Integer)dp.key) <= (Integer)upperBound) {
						values.add(dp.value);
					}
				}
				else if(dp.key instanceof Double && lowerBound instanceof Double && upperBound instanceof Double ) {
	...
	public class LeafNode extends Node {
		...
		//Checks if the leaf node contains the specified key
	...
	public class DictionaryPair implements Comparable<DictionaryPair> {
		 ...
		 */
		public DictionaryPair(Object key, double value) {
			this.key = key;
			this.value = value;
		}
		...
		public int compareTo(DictionaryPair o) {
		    if (key.getClass() != o.key.getClass()) {
		        // If the classes are different, compare based on class names
		        return key.getClass().getName().compareTo(o.key.getClass().getName());
		    }

		    // Now both keys are of the same type
		    if (key instanceof Integer) {
		        return ((Integer) key).compareTo((Integer) o.key);
		    } else if (key instanceof String) {
		        return ((String) key).compareTo((String) o.key);
		    } else {
		        // Since keys are of the same type, we can safely cast to Double
